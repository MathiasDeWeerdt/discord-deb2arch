...
push_to_aur() {
  ...
  local aur_url tmpdir
  aur_url=$(git remote get-url aur)

  # Clone the existing AUR repo so we build on top of its history.
  # AUR rejects force pushes and any repo containing subdirectories.
  tmpdir=$(mktemp -d)
  git clone "${aur_url}" "${tmpdir}"
